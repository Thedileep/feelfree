// src/pages/TherapistAuditLogs.jsx
import React, { useEffect, useState } from "react";
import axios from "axios";
import { toast } from "react-toastify";

const TherapistAuditLogs = () => {
  const [therapistLogs, setTherapistLogs] = useState([]);
  ...
  const fetchTherapistLogs = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/admin/therapist-audit-logs`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setTherapistLogs(res.data.logs || []);
    } catch (err) {
      console.error("❌ Failed to fetch therapist audit logs:", err);
      toast.error("Failed to fetch therapist audit logs");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8">
      <h1 className="text-xl sm:text-2xl font-bold mb-6 text-center text-blue-700">
      ...
      <div className="bg-white shadow-lg rounded-lg p-2 sm:p-4 overflow-x-auto">
        <table className="min-w-full border-collapse">
          <thead className="bg-gray-200 text-left text-xs sm:text-sm">
            <tr>
              <th className="py-3 px-4">Therapist Details</th>
          ...
          <tbody>
            {therapistLogs.map((log) => (
              <tr key={log._id} className="border-b hover:bg-gray-50 align-top">
                <td className="py-2 px-4">{renderDetails(log.therapistId)}</td>
...
export default TherapistAuditLogs;
